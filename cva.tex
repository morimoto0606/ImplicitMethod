...
\subsection{Estimation of the difference of semigroup between the two points}
We estimate the difference of Expectation operator two different points.
...
So our assertion follows from Lemma \ref{discLem1}.
 ...
 \section{Discretization error estimation}\label{sec:disc}
...
\end{align*}
Then by Proposition\ref{discProp}, there is a constant $C>0$ such that\\
\begin{align*}
&|c_0- c_{\Delta}| \leqq   C\sum_{k=1}^K \sum_{i=i_{(k-1)}}^{i_{(k)}-1} \int_{t_i}^{t_{i+1}} dt \int_{t_i}^t (r^{-1/2}+(T_k-r)^{-1/2})dr\\
\leqq &C|\Delta|  \sum_{k=1}^K \sum_{i=i_{(k-1)}}^{i_{(k)}-1}\int_{t_i}^{t_{i+1}} (r^{-1/2}+(T_k-r)^{-1/2})dr. 
\end{align*}
\qed


\section{Property of Stochastic Mesh Operator}\label{sec:sm}
...
\section{Proof of Theorem \ref{main2} and Theorem \ref{main1}}\label{sec:proof}
...
\bibliography{/Users/morimoto/Documents/Reserch/bib/cva} 
